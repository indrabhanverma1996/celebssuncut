   ...
   <div class="row feed-body">
      <div class="col-md-12">
         <div class="uncutcontentcreator">
            <a href="">
               <img src="{{url('/')}}/public/front_assets/images/h1.jpg" alt="image">
               <h1>Content <span>Creators</span></h1>
            </a>
         </div>
         <div class="card w-100 shadow-none bg-transparent bg-transparent-card border-0 p-0 mb-0">
            <div class="owl-carousel reel_bg  category-card owl-theme overflow-hidden nav-none">
               ...
               <div class="item">
                  <?php $extension = substr($postmediavideo->video, -3);
                     $postuser = DB::table('users')->where('id',$postmediavideo->user_id)->first();
                  ...
                  <div data-bs-toggle="modal" data-bs-target="#Modalstory" class="ppp card w125 h200 d-block border-0 shadow-xss rounded-xxxl bg-gradiant-bottom overflow-hidden cursor-pointer mb-3 mt-3">
                     <div class="video_bg">
                        ...
                        <video loop autoplay muted  class="float-right w-100">
                           <source src="public/{{$postmediavideo->video}}">
                        </video>
                        @else
                        <img src="{{url('/')}}/public/images/posts/{{$postmediavideo->video}}" height="360px" width="200px">
                        @endif
                        <div class="card-body d-block  w-100 position-absolute bottom-0 text-center">
                           <a href="#">
                              @if(!empty($postuser->social_profile_image!='' ))
                           ...
                           <a href="{{url('profile/'.$parameters)}}" style="color:#f2f2f2;">{{$postuser->name}}</a></h4>
                           </a>
                        </div>
                     </div>
                  </div>
      ...
      <div class="col-xl-6 col-xxl-6 col-lg-6">
         <div class="card w-100 shadow-xss rounded-xxl border-0 ps-4 pt-4 pe-4 pb-3 mb-3" >
            ...
            <div class="card-body p-0 mt-3 position-relative">
               @if(Auth::user()->social_profile_image!='')
               <figure class="avatar position-absolute ms-2 mt-1 top-5"><img src="{{Auth::user()->social_profile_image}}" alt="image" class="shadow-sm rounded-circle w30"></figure>
               @elseif(!empty(Auth::user()->profile_image))
               <figure class="avatar position-absolute ms-2 mt-1 top-5"><img src="{{url('/')}}/public/images/{{Auth::user()->profile_image}}" alt="image" class="shadow-sm rounded-circle w30"></figure>
               @else
         ...
         <div class="card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3 card-effect-none">
            ...
            <div class="cardseperatore">
               <div class="card-body p-0 d-flex">
                 ...
                 if(!empty($onfido) || !empty($onfido->result) =='clear'){
                     $purchagedPost = DB::table('tbl_post_payment')->where('post_id',$postval->id)->where('expiry_date','>',$now)->where('user_id',Auth::user()->id)->first();
                   }else{
                  $purchagedPost = null;
                   }
                     ...
                     elseif($postval->post_cat_id==1)
                     {
                     $purchagedPost = DB::table('tbl_subscriber')->where('user_id',Auth::user()->id)->first();
                  }
                        else{
                     $purchagedPost = DB::table('tbl_post_payment')->where('post_id',$postval->id)->first();
                  }
                      ?>
                  @if($users->social_profile_image!='')
                  <figure class="avatar me-3"><a href="{{url('profile/'.$parameter)}}"><img src="{{ $users->social_profile_image}}" alt="image" class="shadow-sm rounded-circle w45"></a></figure>
                  @elseif(!empty($users->profile_image))
                  <figure class="avatar me-3"><a href="{{url('profile/'.$parameter)}}"><img src="{{url('/')}}/public/images/{{ $users->profile_image}}" alt="image" class="shadow-sm rounded-circle w45"></a></figure>
                  @else
                  <figure class="avatar me-3">><a href="{{url('profile/'.$parameter)}}"><img src="{{url('/')}}/public/front_assets/images/default-profile-pic-e1513291410505.jpg" alt="image" class="shadow-sm rounded-circle w45"></a></figure>
                  @endif
                  <h4 class="fw-700 text-grey-900 font-xssss mt-1"><a href="{{url('profile/'.$parameter)}}" style="color:black;">{{$users->name}}</a> <span class="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">@php $date  = strtotime($postval->published_at);  $year = date("Y");  @endphp @if($year==date('Y', $date))
               {{date('M-d', $date)}}
                  @else
                  {{date('d-M-Y ', $date)}}
                  @endif </span></h4>
               ...
               <div class="card-body d-block p-0">
                  <div class="row ps-2 pe-2 multi-photo">
                     @php  $postimage = DB::table('tbl_post_images')->where('post_id',$postval->id)->get();
               ...
               <div  class="commentboxall">
                  <div class="img-name">
                     @if(!empty($comment->social_profile_image!='') )
                     <figure class="avatar me-3"><img src="{{$comment->social_profile_image}}" alt="image" class="shadow-sm rounded-circle w35"></figure>
...
@endsection
@include('front.modal.purchagePost')
